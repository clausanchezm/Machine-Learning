...
#       first time.
#
    delta_one = np.zeros(np.shape(Theta1))
    delta_two = np.zeros(np.shape(Theta2))

    for i in range(m): 
        #a1 = np.append(np.ones(m,1), X[i, :], axis= 1) 
        a1 = np.append([1], X[i]).reshape(-1, 1)
 #maybe change order 
        #z2 = np.dot(a1, Theta1.T)
        z2 = np.dot(Theta1, a1)
        a2 = sigmoid(z2)
        a2 = np.append([1], a2).reshape(-1, 1)
       # a2 = np.append(np.ones((m, 1)), a2, axis= 1)
        ...
        for k in range(num_labels):
            if y[i] == k+1: #matlab indexing
                y_i = 1
                
        #y_i = y_i.reshape(-1, 1)
        
        delta3 = a3 - y_i
        #removing bias term out of tehta 
        delta2 = np.dot(Theta2[:,:1].T, delta3) * sigmoidGradient(z2)
        
        #delta2 = delta2[1:]
        delta_one = delta_one + np.dot(delta2, a1.T) 
        delta_two = delta_two + np.dot(delta3, a2.T)

# -------------------------------------------------------------

# =========================================================================
    Theta1_grad = delta_one /m
    Theta2_grad = delta_two /m 
